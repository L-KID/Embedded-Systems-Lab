...
NORMAL_API DSP_STATUS pool_notify_Execute (IN Uint32 numIterations, Uint8 processorId)
{
  ...
  int fixed[128];
  for(int r = 0; r < ms.target_model.rows * ms.target_model.cols; r++) {
    
      fixed[r] = FloatToFixed(ms.target_model.at<float>(r/ms.target_model.cols, r%ms.target_model.cols));             
    
  }
  //memcpy(&pool_notify_DataBuf[0 + 2*sizeof(int)], fixed, 128 * sizeof(int));

  std::copy(fixed, fixed+128, &pool_notify_DataBuf[0 + 2*sizeof(int)]);
  ...
  for(fcount=0; fcount<TotalFrames; ++fcount)
  {
      ...
      for(int iter=0;iter<ms.cfg.MaxIter;iter++)
      {
        ...
        std::copy(roi.data, roi.data+roi_size, &pool_notify_DataBuf[0]);

        roi = bgr_planes[1](ms.target_Region).clone();
        //memcpy(&pool_notify_DataBuf[roi_size], roi.data, roi_size);
       std::copy(roi.data, roi.data+roi_size, &pool_notify_DataBuf[roi_size]);

        roi = bgr_planes[2](ms.target_Region).clone();
        //memcpy(&pool_notify_DataBuf[2 * roi_size], roi.data, roi_size);
        std::copy(roi.data, roi.data+roi_size, &pool_notify_DataBuf[2 * roi_size]);
        ...
        int fixed[128];
        for(int c = 0; c < target_candidate.rows * target_candidate.cols; c++) {
          
            fixed[c] = FloatToFixed(target_candidate.at<float>(c/target_candidate.cols, c%target_candidate.cols));
          
        }

        //memcpy(&pool_notify_DataBuf[3 * roi_size], fixed, 128 * sizeof(int));
        std::copy(fixed, fixed+128, &pool_notify_DataBuf[3 * roi_size]);
