...
Int Task_execute (Task_TransferInfo * info)
{
  ...
  while (command != 0) {
    ...
    case 4: 
     ...
     bgr_planes[1] = &buf[target_region.width * target_region.height * sizeof(unsigned char)];
     if(bgr_planes[1] == NULL) {
        NOTIFY_notify(ID_GPP,MPCSXFER_IPS_ID,MPCSXFER_IPS_EVENTNO,(Uint32)666);
      }

     bgr_planes[2] = &buf[2 * target_region.width * target_region.height * sizeof(unsigned char)];
