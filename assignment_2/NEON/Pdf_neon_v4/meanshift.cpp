...
cv::Mat MeanShift::pdf_representation(const cv::Mat &frame, const cv::Rect &rect)
{
    ...
    float32_t cp_value[12], temp_model[12];
    ...
    for(int i=0;i<rect.height;i++)
    {
	    ...
	    memcpy(&cp_value[0], &curr_pixel_value_A, 12);
	    memcpy(&cp_value[3], &curr_pixel_value_B, 12);
	    memcpy(&cp_value[6], &curr_pixel_value_C, 12);
	    memcpy(&cp_value[9], &curr_pixel_value_D, 12);
